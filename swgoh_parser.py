...
class SWGOHParser():
    ...
    def json_creater(self, directory, allycode, json):
        json_file = "json/" + directory + "/" + str(allycode) + "-" + directory + ".json"
        f = open(json_file, "w")
        f.write(json.dumps(json, indent=2))
        f.close()
   
    def get_player_characters(self, allycode):
        self._setup(allycode)
        player_dict = json_parsing.get_player_characters(allycode)
        player_list = []
